...
)
 {
  companion object {
    fun fromList(pigeonVar_list: List<Any?>): ComposeVideoRequest {
      ...
      val outputWidth = pigeonVar_list[17] as Long?
      val outputHeight = pigeonVar_list[18] as Long?
      val backgroundColorArgb = pigeonVar_list[19] as Long?
      return ComposeVideoRequest(taskId, inputVideoPath, outputVideoPath, watermarkImage, text, anchor, margin, marginUnit, offsetX, offsetY, offsetUnit, widthPercent, opacity, codec, bitrateBps, maxFps, maxLongSide, outputWidth, outputHeight, backgroundColorArgb)
    }
  }
  fun toList(): List<Any?> {
    return listOf(
